...
function calculate() {
    ...
    let inputNumberSpan = document.querySelector('.input-number-span');
    ...
    codeTag.innerHTML = `<pre>${codeDecimalToDecimal}</pre`;

    if(numberInput == null || selectFrom == null || selectTo == null 
        || result == null || error == null || labelResult == null 
        || spanResultNumber == null || inputNumberSpan == null
        || textAreaInputSpanNumber == null || convertDescriptionFrom == null
        || convertDescriptionTo == null || codeTag == null) {
    ...
    function selectedTo() {
        ...
        if(spanConvertDecriptionTo == null) {
            throw new Error('Missing DOM element!');
        }

        if(selectTo.value == 'decimal') {
            changeLabelAndSpan(labelResult, spanResultNumber, spanConvertDecriptionTo, 'Decimal', 'Result: Decimal number', 10, true);
        } else if(selectTo.value == 'binary') {
            changeLabelAndSpan(labelResult, spanResultNumber, spanConvertDecriptionTo, 'Binary', 'Result: Binary number', 2, true);
        } else if(selectTo.value == 'hexadecimal') {
            changeLabelAndSpan(labelResult, spanResultNumber, spanConvertDecriptionTo, 'Hexadecimal', 'Result: Hexadecimal number', 16, true);
        } else if(selectTo.value == 'octal') {
            changeLabelAndSpan(labelResult, spanResultNumber, spanConvertDecriptionTo, 'Octal', 'Result: Octal number', 8, true);
        }
    }
    ...
    function changeLabelAndSpan(label, span, spanDescription, descriptionContent, labelContent, spanContent, isTo) {
        ...
        if(isTo) {
            ...
            inputNumberSpan.textContent = spanContent;
        ...
        if(selectFrom.value == "decimal" && selectTo.value == "decimal") {
            ...
            codeTag.innerHTML = `<pre>${codeDecimalToBinary}</pre`;
    ...
    function mainConvert() {
        ...
        if(from == 'decimal' && to == 'decimal') {
            isConverted = convertIt(Number(numberInput.value), decimalToDecimal, Number.isInteger, textError[0]);
        } else if(from == 'decimal' && to == 'binary') {
            isConverted = convertIt(Number(numberInput.value), decimalToBinary, Number.isInteger, textError[0]);
        } else if(from == 'decimal' && to == 'hexadecimal') {
            isConverted = convertIt(Number(numberInput.value), decimalToHexadecimal, Number.isInteger, textError[0]);
        } else if(from == 'decimal' && to == 'octal')   {
            isConverted = convertIt(Number(numberInput.value), decimalToOctal, Number.isInteger, textError[0]);
        }  else if(from == 'binary' && to == 'binary') {
            isConverted = convertIt(numberInput.value, binaryToBinary, checkInputIsBinary, textError[1]);
        } else if(from == 'binary' && to == 'decimal') {
            isConverted = convertIt(numberInput.value, binaryToDecimal, checkInputIsBinary, textError[1]);
        } else if(from == 'binary' && to == 'hexadecimal') {
            isConverted = convertIt(numberInput.value, binaryToHexadecimal, checkInputIsBinary, textError[1]);
        } else if(from == 'binary' && to == 'octal') {
            isConverted = convertIt(numberInput.value, binaryToOctal, checkInputIsBinary, textError[1]);
        } else if(from == 'hexadecimal' && to == 'hexadecimal') {
            isConverted = convertIt(numberInput.value, hexadecimalToHexadecimal, checkInputIsHexadecimal, textError[2]);
        } else if(from == 'hexadecimal' && to == 'decimal') {
            isConverted = convertIt(numberInput.value, hexadecimalToDecimal, checkInputIsHexadecimal, textError[2]);
        } else if(from == 'hexadecimal' && to == 'binary') {
            isConverted = convertIt(numberInput.value, hexadecimalToBinary, checkInputIsHexadecimal, textError[2]);
        } else if(from == 'hexadecimal' && to == 'octal') {
            isConverted = convertIt(numberInput.value, hexadecimalToOctal, checkInputIsHexadecimal, textError[2]);
        } else if(from == 'octal' && to == 'octal') {
            isConverted = convertIt(Number(numberInput.value), octalToOctal, Number.isInteger, textError[3]);
        } else if(from == 'octal' && to == 'decimal') {
            isConverted = convertIt(Number(numberInput.value), octalToDecimal, Number.isInteger, textError[3]);
        } else if(from == 'octal' && to == 'binary') {
            isConverted = convertIt(Number(numberInput.value), octalToBinary, Number.isInteger, textError[3]);
        } else if(from == 'octal' && to == 'hexadecimal') {
            isConverted = convertIt(Number(numberInput.value), octalToHexadecimal, Number.isInteger, textError[3]);
        }

        //result show output
        if(isConverted != undefined) {
            result.value = isConverted;
            document.querySelector('.text-area-result').textContent = isConverted;
        }   
    }
